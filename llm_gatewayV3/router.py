...
# Check for Gemini Pro plan or custom environment variable overrides
is_gemini_pro = os.getenv("GEMINI_PLAN", "").lower() in ("pro", "paid")
default_gemini_rpm = 1000 if is_gemini_pro else 15
default_gemini_rpd = 1000000 if is_gemini_pro else 1000
default_gemini_tpm = 4000000 if is_gemini_pro else 250000
default_gemini_cooldown = 0 if is_gemini_pro else 4
...
LIMITS = {
    ...
    "gemini":     {
        "rpm": int(os.getenv("GEMINI_RPM", str(default_gemini_rpm))),
        "rpd": int(os.getenv("GEMINI_RPD", str(default_gemini_rpd))),
        "tpm": int(os.getenv("GEMINI_TPM", str(default_gemini_tpm))),
        "cooldown": float(os.getenv("GEMINI_COOLDOWN", str(default_gemini_cooldown))),
        "max_ctx": int(os.getenv("GEMINI_MAX_CTX", "2000000" if is_gemini_pro else "1000000"))
    },
...
class RateState:
    ...
    def snapshot(self, limits):
        ...
        tpm = sum(t for _, t in self.tokens_minute)
        rpm_wait = 0.0
        if len(self.calls_minute) >= limits["rpm"] and self.calls_minute:
            rpm_wait = max(0.0, 60.0 - (now - self.calls_minute[0]))
        tpm_wait = 0.0
        if tpm > limits["tpm"] and self.tokens_minute:
            tpm_wait = max(0.0, 60.0 - (now - self.tokens_minute[0][0]))
        return {
