...
def _backoff_for(err: Exception, has_model_override: bool = False):
    ...
    if status and 500 <= status < 600: return 2, f"upstream {status}"
...
async def chat(req: ChatRequest):
    ...
    # When the only blocker is cooldown or backoff, wait briefly rather than 503-ing.
    if len(candidates) == 1:
        ...
        while time.time() < deadline:
            ...
            wait_time = max(
                snap.get("cooldown_remaining", 0),
                snap.get("backoff_remaining", 0),
                snap.get("rpm_wait", 0),
                snap.get("tpm_wait", 0)
            )
            if wait_time <= 0 or wait_time > 90:
                break
            await _asyncio.sleep(min(wait_time + 0.1, 5.0))
...
_gemini_embed_lock = asyncio.Lock()
_last_gemini_embed_time = 0.0


@app.post("/v1/embed", response_model=EmbedResponse)
async def embed(req: EmbedRequest):
    global _last_gemini_embed_time
    t0 = time.time()
    text = req.text
    ...
    try:
        ...
        ollama_body = {
            "model": EMBED_OLLAMA_MODEL,
            "prompt": text
        }
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(ollama_embed_url, json=ollama_body)
            if r.status_code == 200:
                data = r.json()
                if "embedding" in data:
                    latency = int((time.time() - t0) * 1000)
                    db.log_call(provider="ollama", model=EMBED_OLLAMA_MODEL, status="ok", latency_ms=latency, call_role="embed")
            ...
            # Try /api/embed fallback if /api/embeddings failed or returned non-200
            ollama_embed_url_v2 = f"{OLLAMA_URL}/api/embed"
            ollama_body_v2 = {
                "model": EMBED_OLLAMA_MODEL,
                "input": text
            }
            if task_type:
                mapped = "search_document" if "document" in task_type.lower() else "search_query"
                ollama_body_v2["task_type"] = mapped
            r = await client.post(ollama_embed_url_v2, json=ollama_body_v2)
            if r.status_code == 200:
                data = r.json()
                embedding = None
    ...
    if not GEMINI_API_KEY:
        raise HTTPException(500, "Ollama embedding failed and GEMINI_API_KEY is not configured.")

    try:
        gemini_task_type = None
        if task_type:
            val = task_type.upper()
            if val in ("RETRIEVAL_QUERY", "RETRIEVAL_DOCUMENT", "SEMANTIC_SIMILARITY", "CLASSIFICATION", "CLUSTERING"):
                gemini_task_type = val
            elif "query" in task_type.lower():
                gemini_task_type = "RETRIEVAL_QUERY"
            elif "document" in task_type.lower():
                gemini_task_type = "RETRIEVAL_DOCUMENT"

        gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/{EMBED_FALLBACK_MODEL}:embedContent?key={GEMINI_API_KEY}"
        gemini_body = {
            "model": f"models/{EMBED_FALLBACK_MODEL}",
            "content": {
                "parts": [{"text": text}]
            },
        ...
        if gemini_task_type:
            gemini_body["taskType"] = gemini_task_type

        # Lock / rate budget to protect Gemini free tier (minimum 1 sec between calls)
        async with _gemini_embed_lock:
            now = time.time()
            elapsed = now - _last_gemini_embed_time
            if elapsed < 1.0:
                await asyncio.sleep(1.0 - elapsed)
            _last_gemini_embed_time = time.time()

        backoff = 2.0
        for attempt in range(4):
            try:
                async with httpx.AsyncClient(timeout=30) as client:
                    r = await client.post(gemini_url, json=gemini_body)
                    ...
                    if not values:
                        raise ValueError(f"No values found in Gemini response: {data}")
                    
                    latency = int((time.time() - t0) * 1000)
                    db.log_call(provider="gemini", model=EMBED_FALLBACK_MODEL, status="ok", latency_ms=latency, call_role="embed")
                    return EmbedResponse(
                        embedding=values,
                ...
                if attempt == 3:
                    db.log_call(provider="gemini", model=EMBED_FALLBACK_MODEL, status="error", error=str(e), latency_ms=int((time.time() - t0) * 1000), call_role="embed")
                    raise e
                await asyncio.sleep(backoff)
                backoff *= 2.0
    except Exception as e:
        raise HTTPException(502, f"Embedding failed. Ollama failed and Gemini fallback failed with error: {e}")
